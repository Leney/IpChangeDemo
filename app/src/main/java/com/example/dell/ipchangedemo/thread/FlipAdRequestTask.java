 ...
 * 弹窗广告请求任务对象
 */
public class FlipAdRequestTask implements Runnable {
    ...
    public FlipAdRequestTask(String adunitid, int maxExecuteTime, int adWidth, int adHeight) {
        ...
        NetUtil.doRequestAds(adunitid, Constance.curDeviceInfo, Constance.curIpAddress, adWidth, adHeight, false, new NetUtil.OnAdsResponseListener() {
            ...
            public void onLoadSuccess(JSONObject resultObject, int adWidth, int adHeight) {
                ...
                if (executeTime > 0) {
                    ...
                    int sleepTimeRandom = Tools.randomMinMax(1000, 4565);
                    try {
                        // 休眠随机的时间
                        Thread.sleep(sleepTimeRandom);
                    } catch (InterruptedException e) {
                    ...
                    // 创建一个新的广告请求线程
                    FlipAdRequestTask requestAdTask = new FlipAdRequestTask();
                    ...
                    // 不用再去请求一次此类广告数据了
                    Log.e("llj", "弹窗广告类型的请求已经完毕了！！");
                    Constance.IS_FLIP_AD_DONE = true;
